# TRADE_MATERIAL Engine Wiring Plan

**Status:** READY — execute in fresh session
**Created:** 2026-05-05
**Purpose:** Wire TRADE_MATERIAL into the scenario engine and retire 34 BA_TRADE_MATERIAL band-aids across 17 prime-apply modules. Mirrors the TRADE_OVERHEAD wiring done 2026-05-05 — same architectural pattern, different modifier.
...
`TRADE_MATERIAL` is declared at `Claude/modifiers/TRADE_MATERIAL.json` with this factor table:
...
**Engine wiring is missing.** Currently every `MOD_APPLY_*_PRIME` module carries `modifier_eligibility.material: true` and uses the universal coat tasks (`TSK_BRUSH_COAT_LF/SF`, `TSK_SPRAY_COAT_LF/SF`) but band-aids the rate via per-entry `rate_per_hour` overrides. The band-aids are tagged `_band_aid: "BA_TRADE_MATERIAL"` for grep-based reversion.

**The replacement design** (per the TRADE_MATERIAL.json description):
...
| MOD_APPLY_WOOD_CEILING_PRIME (SF) | 90 | 90 | 70 | 175 | composite (BA_TRADE_OVERHEAD,BA_TRADE_MATERIAL) |

**Behavioral shifts to expect on band-aid removal** (post-TRADE_MATERIAL retirement, with TRADE_OVERHEAD already wired for the composite):
...
|---|---|
| `engine/run-estimate-scenario.js` `computeScenarioModifierStack` is the single point where modifier factors get folded into `total` | TRADE_MATERIAL wiring is one explicit branch alongside the existing qt/height/texture/condition/overhead branches |
| `engine/modifier-registry.js` FALLBACK table is the source for modifier factor lookups when bundle is missing | Need to add TRADE_MATERIAL fallback for safety |
| All affected `_band_aid` strings are exact matches `"BA_TRADE_MATERIAL"` (pure) or `"BA_TRADE_OVERHEAD,BA_TRADE_MATERIAL"` (composite) | Strip script discriminates by exact string match — composite becomes pure-TRADE_OVERHEAD-marker after TRADE_MATERIAL is wired and stripped |
| 20/20 scope-tree smoke must pass before AND after wiring | Same regression check used for TRADE_OVERHEAD |
...
**File touches:**
- Modified: `Claude/tools/paintscope/src/engine/modifier-registry.js` — add TRADE_MATERIAL to FALLBACK table.
- Modified: `Claude/tools/paintscope/src/engine/run-estimate-scenario.js` — add `deriveMaterialType` helper + explicit TRADE_MATERIAL branch in `computeScenarioModifierStack`. Fold result into `total`. Add `material` and `materialType` to the returned object (mirrors `overhead` and `surfaceOrientation` from TRADE_OVERHEAD wiring).
...
const material = eligibility.material === true
  ? (bundle ? getFactor(bundle, 'TRADE_MATERIAL', material_type)
            : (FALLBACK_TABLE.TRADE_MATERIAL.factors[material_type] ?? 1.0))
  : 1.0;
...
## Phase 2: Strip pure TRADE_MATERIAL band-aids

**Scope:** the 16 modules with pure `BA_TRADE_MATERIAL` (32 entries — 14 LF prime modules × 2 entries each + 2 SF wall/wainscot prime modules × 2 entries each).

**Excluded from this phase:** `MOD_APPLY_WOOD_CEILING_PRIME` (2 composite entries) — handled in Phase 3.

**Strip script logic** (modeled on the TRADE_OVERHEAD strip from 2026-05-05):
```js
for (const mod of targetModules) {
  for (const t of mod.tasks) {
    if (t._band_aid === 'BA_TRADE_MATERIAL') {
...
After stripping: re-run scope-tree smoke; lab-verify a baseboard prime estimate produces hours that match the post-TRADE_MATERIAL math (canonical_rate / 1.25).

**Doctrine update:** for each stripped module, replace the "until TRADE_MATERIAL engine wiring lands" verbiage with "TRADE_MATERIAL modifier wired 2026-MM-DD — eligibility.material=true triggers WB_PRIMER 1.25× time multiplier. Effective primer rates: brush=N/1.25, spray=M/1.25." Keep the doctrine succinct.
...
**Scope:** 1 module, 2 entries — both currently flagged `BA_TRADE_OVERHEAD,BA_TRADE_MATERIAL`.

After Phase 1+2 ships, TRADE_OVERHEAD is already wired (done 2026-05-05) and TRADE_MATERIAL is now wired. Both penalties fire automatically when:
- `eligibility.overhead === true` AND surface orientation is CEILING → 1.25× from TRADE_OVERHEAD
- `eligibility.material === true` → 1.25× from TRADE_MATERIAL
...
```js
for (const t of mod.tasks) {
  if (t._band_aid === 'BA_TRADE_OVERHEAD,BA_TRADE_MATERIAL') {
    delete t.rate_per_hour;
    delete t._band_aid;
  }
}
...
**Doctrine update on `MOD_APPLY_WOOD_CEILING_PRIME`:** replace the "until TRADE_OVERHEAD + TRADE_MATERIAL engine wiring lands" verbiage with the post-wiring target — "TRADE_OVERHEAD + TRADE_MATERIAL both wired. Combined 1.5625× time multiplier. Effective rates: brush=44.8, spray=112 SF/hr."
...
- **TSK_TRIM_SPOT_PRIME_KNOTS**-related routing — already retired (round-31 hard retirement). Not a TRADE_MATERIAL concern.
...
- **Modifier interactions beyond TRADE_OVERHEAD × TRADE_MATERIAL.** No other compositions exist in the canonical bundle today.
- **Engine refactor.** Same code-change shape as TRADE_OVERHEAD wiring — incremental branch addition. No structural changes to modifier-stack / scenario-engine.
...
2. Verify the architectural assumptions table — confirm `computeScenarioModifierStack` signature matches the TRADE_OVERHEAD wiring, confirm FALLBACK table location, confirm 17 module band-aid sites still match (run the audit script in the Context section).
3. Phase 1: add FALLBACK row + `deriveMaterialType` helper + `material` branch alongside the TRADE_OVERHEAD branch in `computeScenarioModifierStack`. Run `node Claude/scripts/smoke-scope-tree.mjs` — must remain 20/20.
...
5. Phase 2: run the strip script over the 16 pure-TRADE_MATERIAL modules. Regen bundle. Smoke must remain 20/20.
...
8. Browser-verify: load a project with at least one bare-substrate baseboard or wood-ceiling. Confirm prime hours match the TRADE_MATERIAL-adjusted math.
