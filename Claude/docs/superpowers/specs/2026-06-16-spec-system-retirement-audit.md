...
## 14. Phase 6 — DONE (2026-06-17)

**Phase 6 (delete the spec machinery) landed** on `claude/cranky-saha` per the plan [`2026-06-17-spec-retirement-p6-delete-machinery.md`](../plans/2026-06-17-spec-retirement-p6-delete-machinery.md). Executed subagent-driven (implementer + two-stage review on the substantive tasks; controller-verified on the trivial single-file deletions). **7 commits `1cc6780` → `a6b3bb2` → `6f24e7f` → `23bad19` → `c113062` → `eed41d3` → `a082f9d`**; final comprehensive review APPROVED.

- **T1 `1cc6780`** — deleted orphan `engine/scenario-to-spec-results.js` (0 importers) + fixed the stale §7a pipeline block in `tools/paintscope/CLAUDE.md`.
- **T2 `a6b3bb2`** (+ follow-up `6f24e7f`) — removed the vestigial `db`/`specData` param thread from the scenario engine end-to-end (16 files). **The audit/plan under-enumerated the thread:** the implementer also had to drop `db` from 3 internal helpers the listed fns passed it onward to — `buildElevationScenarioInputs` / `buildStandaloneScenarioInputs` (context-adapter) + `classifyFloorProtectionTask` (floor-protection); each verified to never read `db` (caught via a vitest `ReferenceError`, interior-only — floor-protection is interior-only). Follow-up `6f24e7f` cleaned the dead `@param specData` JSDoc on `resolvePassGroups` + 27 vestigial `null` 3rd-args in 2 test files + a stale comment.
- **T3 `23bad19`** (+ follow-up `c113062`) — deleted the Rates + Assemblies UI (`components/rates/*` 6, `components/assemblies/*` 3), `spec-editor-reducer.js`, `spec-editor-db.js`, `useSpecData.jsx`, `useAssemblies.js` + App.jsx nav/routes/imports/`SpecDataProvider` wrapper + LabRoot wrapper. Code-review caught `data/assembly-db.js` (the Assemblies IDB layer, sole consumer was the deleted `useAssemblies.js`) → follow-up `c113062` deleted it.
- **T4 `eed41d3`** — deleted `src/data/db-bundle.js` (the spec database). **Headline proof: parity 21.13/1.73 EXACT with the entire spec DB gone** — the engine reads `scenario-rate-data.js` (P3) + `scenario-bundle.gen.js`, nothing from db-bundle.
- **T5 `a082f9d`** — deleted the build-only generators `import_spec.py` + `export_db_bundle.py` + `validate_specs.py` + (newly found) `build-scenario-rate-data.mjs` (the JS generator that froze `scenario-rate-data.js` from the now-deleted db-bundle); archived the `/specs` authoring infra (`_registry`/`_schemas`/`_templates`/`_backlog`/`Spec Catalog`/`CLAUDE.md`) into `_archive/spec-system/` (Claude/specs/ now empty); rewrote the `scenario-rate-data.js` header (no longer auto-generated — frozen module, restorable via the tag).

**Verification (every task, EXACT throughout):** `npx vitest run` → **168**; `npx vite build` → green (**258 → 243** modules; the 15-module drop is all from T3's UI deletion — orphan/build-only deletions like T1/T4/T5 were already out of the module graph, a 0-module change); NC-interior parity **21.13** / exterior **1.73**.

**Module-graph note (corrects the plan's per-task count predictions):** Vite only counts modules in the import graph, so deleting a file with zero *importers* (the T1 orphan, T4's db-bundle after its importers were severed, T5's build-only scripts) is a **0-module** change — only T3 (imported via App.jsx) moved the count. The hard gates (green build + exact parity + 168) are authoritative.

**Material golden (P3 §12 deferred item) — confirmed SEPARABLE, not done here.** The existing `material-estimates.test.js` already asserts the *complete* `materialEstimates` list for `p2a-int.json` equals exactly the 4 drywall entries (baseboard, a non-drywall spec in the fixture, emits none) — so it already guards the full material path through every Phase-6 deletion, and T4 proved (exact parity) that db-bundle deletion is material-neutral because `scenario-rate-data.js` (untouched by Phase 6) is the source. Activating a non-drywall spec that emits *non-zero* material (builtin/cabinet/stair) needs new fixture substrates — a standalone fixture-authoring effort, not a deletion. Remains a follow-up.

**Leftovers (NOT defects; future cleanup):**
1. **The rest of `Claude/database/`** — T5 deleted only the 3 spec-pipeline generators; the other db-maintenance scripts (`fix_null_rates_interior.py`, `remove_mdf_edge_seal.py`, `split_teardowns.py`, `validate_db.py`, `query_examples.sql`), `imports/` reports, and `exports/db_bundle.js` are now-vestigial SQLite-spec-db tooling, left untouched (un-vetted individually). A future pass can archive/delete the whole `Claude/database/` tree.
2. **`tools/paintscope/CLAUDE.md` is broadly stale** — its §2 project-structure tree still lists `db-bundle.js`, `run-estimate.js`, `spec-resolution.js`, `modifier-stack.js`, `spec-maps.js`, `spec-compatibility.js` (all deleted across P1–P3 + Phase 6) and "Spec table viewer/editor" under Future Possibilities. Was already stale pre-Phase-6; warrants a dedicated refresh.

**Retirement status: P1 + P2a + P2b + P3 + Phase 4 + Phase 5 + Phase 6 DONE.** The Scenario Engine is the SOLE estimation path AND the branch carries NO spec machinery (no db-bundle, no spec tables, no Rates/Assemblies UI, no `db` param, no spec-pipeline scripts; `/specs` archived). Remaining: **Phase 7** — merge `claude/cranky-saha` → `main`. **Branch stays as-is — do NOT merge until the user authorizes Phase 7.**
