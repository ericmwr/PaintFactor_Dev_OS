...
**Now-orphaned spec-bundle tables** (no remaining reader once the legacy engine died — informational, for the Phase-6 delete): `spec_required_inputs`, `sop_modules`, `sop_tasks`, `task_production_rates`, `coat_counts`, `factor_modifiers`. The scenario engine never read them; they sit unused in `db-bundle.js` until Phase 6.
