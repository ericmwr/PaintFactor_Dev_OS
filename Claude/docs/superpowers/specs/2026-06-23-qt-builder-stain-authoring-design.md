...
1. **Pilot — `int_door_casing`** end-to-end: select it → Phase=Stain (default From-state=Bare) → vantage grid (prep + apply with coat stepper + interstage) + Materials row (Stain, canonical menu); author a QT4/QT5 coat change (fork) + a per-tier material pick; switch Phase=Clear → From-state=Stained → author its coats/material. **RESULT (2026-06-23): PASS** — live-verified (Playwright), all checklist items green, 0 console errors (coat fork + revert + draft banner, material override, Clear-phase switch all work).
2. **Generalize** — the wiring is generic (`coating_phase` / `dynamic_coats`-driven). **RESULT (2026-06-23): PASS for ALL 22 stain families** (headless resolution check) — every phase resolves with the derived default-state (stain→SS_BARE/3 methods, sealer→SS_STAINED/2, clear→SS_STAINED/2), uniform across flat trim + feature-wood + stair families.
3. **Stairs — VERIFIED COVERED (2026-06-23).** Contrary to the pre-build assumption, stair stain scenarios ARE fully decomposed: `SCN_INT_RRST_*` / `SCN_INT_SRST_*` (STAIN/SEALER/CLEAR + BARE) carry `coating_phase` with `paintable_item` `riser` / `open_rail` / `tread`, `domain: "interior"`, and the identical `dynamic_coats` / `coat_counts` / `material_systems` shape as the flat families. They surface in `listSubstrates` and resolve every phase through the generic P2 wiring — **no separate stair handling is needed.** Residual (pre-existing, orthogonal, deferred): the finer sub-components (baluster/newel/wall_rail/stringer/skirtboard) map to `open_rail`/`riser` via `EXTRA_TOKEN_SPECS` for materials and don't get their own QT Builder substrate rows; the user authors at the `open_rail`/`riser`/`tread` level.
