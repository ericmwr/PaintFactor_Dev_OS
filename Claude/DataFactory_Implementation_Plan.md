...
**Contains:**
- All tables per SQLite Schema Contract v1.1.0:
  - **Spec structure:** spec_families, spec_configuration_dimensions, spec_paintable_item_types, spec_variants, spec_variant_item_inclusions, spec_scope_boundaries, spec_required_inputs, spec_protection_zones, spec_adjacency_declarations, spec_state_declarations, spec_change_log
  - **Materials:** material_systems, material_system_products, material_coverage_profiles, material_consumables
  - **SOP:** sop_round_configurations, sop_modules, sop_tasks
  - **Production:** task_production_rates, factor_modifiers, factor_task_applicability, quality_tier_effects
  - **QA/Governance:** spec_qa_reports
...
| spec.json → scope_boundaries | spec_scope_boundaries |
| spec.json → adjacency_declarations | spec_adjacency_declarations |
| spec.json → protection_zones_required | spec_protection_zones |
| spec.json → state_declarations | spec_state_declarations |
| spec.json → required_paintscope_inputs | spec_required_inputs |
| spec.json → change_log[] | spec_change_log |
| materials.json → material_systems[] | material_systems, material_system_products |
| materials.json → coverage_profiles[] | material_coverage_profiles |
| materials.json → consumables[] | material_consumables |
| sop_modules.json → round_configurations[] | sop_round_configurations |
...
| production.json → height/texture/drywall_level_effects[], factor_modifiers[] | factor_modifiers, factor_task_applicability |
| production.json → quality_tier_effects[] | quality_tier_effects |
| qa_report.json | spec_qa_reports (summary + full JSON blob) |
...
1. **ID uniqueness scope:** OPEN — Are task IDs (TSK_) unique globally or only within a spec family? This affects PK design. Currently they appear to be globally unique via context prefix, but this should be formalized. Current schema uses composite PKs (id, spec_family_id) which allows duplicates across families.

2. **Version coexistence:** RESOLVED — Replace. Importing v2 deletes v1 normalized data. Raw JSON preserved with version tag for rollback.

3. **Cross-spec family references:** OPEN — Some specs reference sibling specs (e.g., prime spec references corresponding finish spec). The `relationships` key in spec.json is currently raw-only (Section 8 of Schema Contract). A `spec_relationships` table is a future candidate.

4. **Controlled enums in DB vs. application:** RESOLVED — Store as reference tables via `seed_enums.sql` for maximum integrity.

5. **Resolution.json handling:** RESOLVED — Store in `spec_artifacts_raw` only. It's a development artifact, not a runtime artifact.

6. **Database location in .gitignore:** RESOLVED — Add `database/paintfactor.db` to .gitignore. Schema, scripts, and registry are tracked.

7. **Factor modifier architecture:** RESOLVED (2026-02-10) — Unified `factor_modifiers` table with `modifier_category` discriminator column. Domain-specific arrays (height_effects, texture_effects, etc.) mapped into the unified table.

8. **Material system products:** RESOLVED (2026-02-10) — Child table `material_system_products` created. Supports future full product catalog + sundries table.

9. **QA report granularity:** RESOLVED (2026-02-10) — Summary + full JSON blob in `spec_qa_reports`. Detailed criterion reviews preserved in raw JSON and `full_report` column. `spec_qa_issues` table removed.

10. **Round configurations:** RESOLVED (2026-02-10) — Dedicated `sop_round_configurations` table with `phase_sequence` as JSON array. Variants FK to `round_id`.
